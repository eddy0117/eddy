...
class RepAllRollCommand extends commando.Command
{
    ...
    {  
        ...
        */ 
        ...
        if(scale == 1)
        {
            reputation[message.author.id].rep = reputation[message.author.id].rep + Math.floor(Math.random() * Math.abs(reputation[message.author.id].rep) * 2) + 1;
            addsub = "你的名聲增加到了"
        }
        else 
        {
            reputation[message.author.id].rep = reputation[message.author.id].rep + Math.floor(Math.random() * Math.abs(reputation[message.author.id].rep) * 2 * -1) - 1;
            addsub = "你的名聲減少到了"
        } 
